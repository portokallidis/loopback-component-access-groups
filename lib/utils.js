...
module.exports = class AccessUtils {
  constructor(app, options) {
    ...
    // Default the userId to the group model name + Id.
    ...
    models.forEach(modelName => {
      ...
      if (typeof Model.observe === 'function') {
        ...
        Model.observe('access', (ctx, next) => {
          ...
          const isAdmin = JSON.parse(JSON.stringify(currentUser)).isAdmin;
          try { 
            isAdmin = JSON.parse(JSON.stringify(currentUser)).isAdmin
          } catch(err){
            // console.log(err);
          }
          if (currentUser) {
            ...
            if (isAdmin) {
              debug('isAdmin: true - skipping access filters')
              return next()
            }
      ...
      Promise.join(this.getCurrentGroupId(context), this.getTargetGroupId(context),
        (currentGroupId, targetGroupId) => {
          if (!currentGroupId) {
            ...
            debug(`ORG ROLE RESOLVER - Could not get group id, skipping ACL check for ${accessGroup}`);
            return [ false ];
          }
          ...
          const conditions = {
            [this.options.userIdKey]: userId,
            [this.options.foreignKey]:  scope.currentGroupId,
            role: roleName
          };
        
          actions.push(GroupAccess.count({
            [this.options.userIdKey]: 'public',  //special key value for "public" access
            [this.options.foreignKey]:  scope.currentGroupId,
            role: roleName
          }));
          ...
          if (targetGroupId && targetGroupId !== scope.currentGroupId) {
        ...
        .spread((publicCount,currentGroupCount, targetGroupCount) => {
          let bAllowAccess = false;
          
          if (publicCount>0) {
            debug('ORG ROLE RESOLVER - Group identified as PUBLIC, so allow passthrough access')
            // No group context was determined, so allow passthrough access.
            bAllowAccess = true;
          } else if (currentGroupCount === false) {
            debug('ORG ROLE RESOLVER - No group context was determined, so allow passthrough access')
            // No group context was determined, so allow passthrough access.
            bAllowAccess = true;
          }
          else {
            ...
            debug(`ORG ROLE RESOLVER - User ${userId} ${bAllowAccess ? 'has role' : 'does not have role'} ${roleName} in group ${scope.currentGroupId}`);
            ...
            if (scope.targetGroupId && scope.targetGroupId !== scope.currentGroupId) {
              ...
              debug(`ORG ROLE RESOLVER - Attempting save into new target group, User ${userId} ${tMember ? 'has role' : 'does not have'} ${roleName} in target group ${scope.targetGroupId}`);
          ...
          // Note the fact that we are allowing access due to passing an ACL.
           if (bAllowAccess) {
            debug('ORG ROLE RESOLVER - Allowing access due to passing an ACL');
            // this.app.loopback.getCurrentContext().set('groupAccessApplied', true);
          }

          debug(`ORG ROLE RESOLVER - ${accessGroup} ACL check: ${bAllowAccess}`);
